...
- **`DestinationStop` 是 StopID 不是站名**（`"306195"`）。所以終點站名不在
  到站資料裡，是另外查路線資料拼出來的 —— 見下面「「往哪裡」是拼出來的」。
...
**這是量出來的**（2026-09-05，直接打 TDX 看回應 header）：
...
每分鐘 5 個，不是常見的每秒幾十個。這個數字小到會改變設計，而且
**它沒有出現在任何文件裡，只在 header 裡** —— 撞到之前只會看到 429。

上面那句「每次重整是 3 個請求」配上 30 秒重整就是 **6 次／分鐘**，
再加台鐵看板是 8 次 —— **光是把交通分頁開著就一定超過**。中繼服務的快取
本來擋掉了大部分，但到站類的 TTL 當時也設 30 秒，跟重整頻率一樣，
等於每次都沒中：中繼自己穩定 6 次／分鐘打上去。所以真機上那個
「交通資料服務忙碌中」一直沒有真正消失，只是變得比較零星。

修的方式是把 TTL 照額度訂（到站 60 秒、台鐵看板 120 秒，穩定 2.5 次／分鐘），
再加一個額度計數：**用完就給備胎，不要打過去**。重點不是省那一次失敗的
請求，是把額度留給沒有備胎可給的請求 —— 到站資料手上一定有舊的一份，
而使用者第一次點進一條路線時，那條路線的站序是真的一份都沒有，
打不到就是一整頁空白（1551 就是這樣壞的）。

**所以直接打 TDX 的那條路（`--dart-define` 帶金鑰、不走中繼）現在必爆**：
一次重整 4 個請求、30 秒一次就是 8 次／分鐘。開發時要驗真實資料的話，
心裡要有數那些 429 是額度不是壞掉。
...
「人工加選尚未開放」這種話**不在頁面上**，是注入一段 JS 讓瀏覽器彈出來，
然後把人踢回首頁。App 不跑 JS，所以在 `parsing/server_message.dart` 之前，
那些話在畫面上是**完全消失**的 —— 使用者看到一頁空白，不知道是 App 壞了
還是自己來錯時間。

那句話在**導向鏈的中間**：派發器 → 內容頁（話在這裡）→ 首頁。
所以要在 `followJsRedirect` 裡邊跟邊收（它吃一個 `onMessage` callback），
呼叫端只看得到頭和尾，兩邊都沒有話。

送出查詢（`runQuery`）和按結果表格裡的動作鈕（`runRowAction`）也各自要收 ——
後者尤其重要，那是會改資料的操作，學校回「加選成功」還是「這門課已額滿」
都走 JS，吞掉的話使用者按完不知道到底成功了沒。

### 難的是不要抓錯

**規則不能是「頁面上有 alert」**，也不能是「有沒有 `Message.showMessage`」。
每一頁的驗證函式裡都寫著這兩種東西：

```js
function doQuery() { ... alert("學期成績-學年期不可為空白!"); }
function doPrint(...) { ... Message.showMessage("必須選擇資料再進行處理!!"); }
```

那些只有使用者按下按鈕、而且條件不符時才會執行。撈出來顯示的話，使用者
一打開成績就看到一句「必須選擇資料」，或在人工加選那頁看到「學年期不可為
空白」—— 完全不相干、但看起來很像真的。

**分辨靠位置**：伺服器注入的（`RegisterStartupScript`）一定在 script 的頂層
直接執行，驗證訊息包在函式裡。逐段 `<script>` 掃，數 `{}` 的淨深度：
人工加選那句是 0、`doPrint` 那句是 3。

括號是硬數的（字串和註解裡的也算），對這個系統的 JS 夠用，而且**算錯的方向
是安全的** —— 深度變成非 0 就是不顯示，回到「跟以前一樣什麼都沒有」。

這一條是**踩到反例才修對的**：第一版用「有沒有 showMessage」，在當時手上的
12 個 fixture 上零誤報，第 13 個（`GRD5010_02` 成績明細）就破了。
**零誤報不等於規則正確，可能只是樣本裡剛好沒有反例。**
...
「還有 12 分鐘下課」「現在這一格」「已上完／下一堂」這些都是 build 當下用
`DateTime.now()` 算的。而這些畫面只在 controller 有變動時重建 —— 開著 App
放五分鐘，那個數字還是 12。**一個過期的「還有 3 分鐘」會害人以為還來得及。**

所以：

- **計時器放頁面層，不要放在各個小 widget 裡**（首頁是 `_HomePageState`、
  課表頁是 `_Body`）。一個計時器重建整棵 subtree，所有時間相關的東西一起
  更新；放在小 widget 裡的話會變成「狀態列跳了、格子還凍著」，而且每個
  widget 各養一個計時器。
- **對齊到整分**：先等到下一個整分、再每 60 秒一次。不對齊的話，剛好在
  「還有 1 分鐘」那一秒進來，會晚快一分鐘才跳成 0。
- **回到前景要補一次 `notifyListeners()`**（`handleResumed`）。Android 會
  凍結切到背景的 isolate，被凍住的計時器不會執行 —— 午休回來那個數字還停
  在出門前。那個方法本來就是這份程式裡「一定會發生」的那道防線。
- 注入了固定時間（測試）就**不要開計時器**。測試驗的是計時器的安全 ——
  跑過幾分鐘不崩、離開頁面不殘留（殘留的話 `testWidgets` 會自己判失敗）。

### 教職員的查詢頁，學生帳號也開得起來

有些功能頁的學號欄是空的，而**空的送出去回來的是整批學生**：`GRD7050`
（缺曠課紀錄）不填條件直接查，拿到 70 個人的學號、姓名和缺曠節數
（2026-09-08 實測）。使用者只是想看自己缺幾節。

`openFunction` 會把**空的** `Q_STNO` 自動填上本人學號（學號由
`AppController` 給，那是權威來源 —— 從 Keystore 自動登入那條路不經過
`completeLogin`）。只填空的：學校已經帶好值的頁面不要動它。
