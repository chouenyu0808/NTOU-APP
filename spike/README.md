...
**2026-09-07 更新：成績系統出現了，這一節原本的結論已經不成立。**

原本寫的是「成績查詢不在學生選單裡，50 個功能全查過」—— 那句話當時是對的，
學校後來一次上架了整個 `GRD` 模組（選單從 50 個長到 131 個）。
`selectors.json` 的 `pages.grades.path` 現在指向 `GRD5010`，不是 `null`。

**但它還沒打通**，而且卡的地方跟「找不到入口」不一樣：
按 `QUERY_BTN1` 查出來的是一列**學生清單**（部別/系所/年級/班級/學號/姓名），
不是成績。成績在明細頁 `GRD5010_02.aspx`，要點那一列的「詳」才進得去 ——
用 `--detail` 跟過去（見「跟著『詳』進明細頁」那一節）。

抓到明細頁之後又卡在另一件事：**這個帳號在本校還沒有任何成績**
（1141/1142 查詢都是「查無符合資料」，畢業資格顯示已得學分 0）。
所以欄位全是空的，**沒有正樣本可以驗 parser**。

在有人真的有成績之前，App 這邊做的是畢業資格（`ENRG010`）——
那一頁的必修課目和應修學分是真實資料，而且成績欄就在同一張表上，
將來有成績時同一個 parser 自動就顯示。
...
- **`DestinationStop` 是 StopID 不是站名**（`"306195"`）。終點站名不在到站
  資料裡 —— App 那邊是拿 `RouteUID` 去查路線的起訖站再靠 `Direction` 挑一邊
  拼出來的（0 看終點欄位、1 看起點欄位，**弄反了不會報錯**，畫面上是一個
  很合理但方向相反的站名）。詳見 CLAUDE.md 的「「往哪裡」是拼出來的」。
