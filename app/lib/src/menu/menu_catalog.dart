...
class AisFunction {
  ...
  /// 這一頁是給系辦看的報表，不是學生的功能。
  ///
  /// 12 個「XX 學生名單查詢」（上課時間衝堂、選課學分超過上限、重複修讀…）
  /// 共用同一支 `TKE2070_01.aspx`，內容是行政統計。學生點進去不是沒權限
  /// 就是空的，而它們佔掉「選課系統」26 個項目裡的 12 個 —— 將近一半。
  ///
  /// **判斷用路徑，不是名字。** 「學生名單查詢」看起來很好認，但那是學校
  /// 隨時會改的文案，改一個字就漏掉 —— 而漏掉的症狀是它又混回清單裡，
  /// 沒有人會發現少擋了一個。路徑是它們真正的共同點。
  ///
  /// 刻意只降級不隱藏：這是「你大概用不到」，不是「你不准看」。
  bool get staffOnly => path.toUpperCase().contains('TKE2070_01.ASPX');
